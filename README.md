...
research/           alternative approaches, each with its own ALGORITHM.md and figures
data/               the three sample renders
output/             committed results (geometric); output/semantic/ (with VLM)
Dockerfile, docker-compose.yml
```

[`research/`](research/README.md) holds offline experiments that sit beside the
pipeline without changing it: a medial-axis replacement for the watershed
markers, and a floor-finish split of open-plan areas that needs no API key.
Each approach documents what it improves and what it breaks, with figures
regenerated by `uv run python research/run_all.py`.
